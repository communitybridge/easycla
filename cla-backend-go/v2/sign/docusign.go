...
func (s *service) GetEnvelopeDocuments(ctx context.Context, envelopeID string) ([]DocuSignDocument, error) {
	f := logrus.Fields{
		"functionName": "v2.GetEnvelopeDocuments",
		"envelopeID":   envelopeID,
	}
	...
	// Create the request
	url := fmt.Sprintf("%s/accounts/%s/envelopes/%s/documents", utils.GetProperty("DOCUSIGN_ROOT_URL"), utils.GetProperty("DOCUSIGN_ACCOUNT_ID"), envelopeID)
