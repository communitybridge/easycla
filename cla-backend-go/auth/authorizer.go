...
func (a Authorizer) SecurityAuth(token string, scopes []string) (*user.CLAUser, error) {
	...
	if !ok {
		log.WithFields(f).Warnf("SecurityAuth - username not found in claims with key: %s", a.authValidator.usernameClaim)
		return nil, errors.New("username not found")
	}

	username, ok := usernameClaim.(string)
	if !ok {
		log.WithFields(f).Warnf("SecurityAuth - invalid username: %+v", usernameClaim)
		return nil, errors.New("invalid username")
	}
	nameClaim, ok := claims[a.authValidator.nameClaim]
	if !ok {
		log.WithFields(f).Warnf("SecurityAuth - name not found, error: %+v", err)
		return nil, errors.New("name not found")
	}
	name, ok := nameClaim.(string)
	if !ok {
		log.WithFields(f).Warnf("SecurityAuth - invalid name, error: %+v", err)
		return nil, errors.New("invalid name")
	}
	emailClaim, ok := claims[a.authValidator.emailClaim]
	if !ok {
		log.WithFields(f).Warnf("SecurityAuth - email not found, error: %+v", err)
		return nil, errors.New("email not found")
	}
	email, ok := emailClaim.(string)
	if !ok {
		log.WithFields(f).Warnf("SecurityAuth - invalid email, error: %+v", err)
		return nil, errors.New("invalid email")
	}
	// Get User by LFID
	lfuser, err := a.userPermissioner.GetUserAndProfilesByLFID(username)
	if err != nil {
		log.WithFields(f).Warnf("SecurityAuth - GetUserAndProfilesByLFID error for username: %s, error: %+v", username, err)
	...
	for _, scope := range scopes {
		...
		case projectScope:
			...
			if err != nil {
				log.WithFields(f).Warnf("SecurityAuth - GetUserProjectIDs error for user id: %s, error: %+v", lfuser.UserID, err)
				return nil, err
			}
		...
		case companyScope:
			...
			if err != nil {
				log.WithFields(f).Warnf("SecurityAuth - GetUserCompanyIDs error for user id: %s, error: %+v", lfuser.UserID, err)
				return nil, err
			}
