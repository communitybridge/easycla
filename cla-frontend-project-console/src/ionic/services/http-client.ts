...
export class HttpClient {
  ...
  get(url) {
    return Observable.fromPromise(this.buildHeaders()).switchMap(headers => this.http.get(url, { headers: headers }));
  }

  post(url, data) {
    return Observable.fromPromise(this.buildHeaders()).switchMap(headers =>
      this.http.post(url, data, { headers: headers })
    );
  }

  put(url, data, contentType: string = 'application/json') {
    return Observable.fromPromise(this.buildHeaders(contentType)).switchMap(headers =>
