...
export class ProjectAnalyticsPage {
  ...
  getDefaults() {
    ...
    this.issuesActivityChart = {
      chartType: 'AreaChart',
      dataTable: [['Date', 'Issues Open', 'Issues Closed']],
      options: {
        hAxis: {
          textStyle: { color: '#0b4e73' },
          gridlines: {
    ...
    this.prsActivityChart = {
      chartType: 'AreaChart',
      dataTable: [['Date', 'PRs Open', 'PRs Merged', 'PRs Closed']],
      options: {
        hAxis: {
          textStyle: { color: '#0b4e73' },
          gridlines: {
  ...
  getCommitActivity(span) {
    ...
    this.analyticsService.getMetrics(index, metricType, groupBy, tsFrom, tsTo).subscribe(metrics => {
      this.commitsActivityChart.dataTable = [['Date', 'Commits']];
  ...
  getcommitsDistribution(span) {
    ...
    this.analyticsService.getMetrics(index, metricType, groupBy, tsFrom, tsTo).subscribe(metrics => {
      this.commitsDistributionChart.dataTable = [['Date', 'Commits']];
      if (!this.isEmpty(metrics.value)) {
  ...
  getIssuesStatus(span) {
    ...
    let issuesStatus;
    this.analyticsService.getMetrics(index, metricType, groupBy, tsFrom, tsTo).subscribe(metrics => {
      this.issuesStatusChart.dataTable = [['Status', 'Issues']];
      if (!this.isEmpty(metrics.value)) {
        Object.entries(metrics.value).forEach(([key, value]) => {
          issuesStatus = value;
        });
  ...
  getIssuesActivity(span) {
    ...
    let tsTo = this.timeNow;
    this.analyticsService.getMetrics(index, metricType, groupBy, tsFrom, tsTo).subscribe(metrics => {
      this.issuesActivityChart.dataTable = [['Date', 'Issues Open', 'Issues Closed']];
      if (!this.isEmpty(metrics.value)) {
        Object.entries(metrics.value).forEach(([date, value]) => {
          date = this.formatDate(date);
          if (value.value['open'] && !value.value['closed'])
            this.issuesActivityChart.dataTable.push([date, value.value['open'], 0]);
          if (!value.value['open'] && value.value['closed'])
            this.issuesActivityChart.dataTable.push([date, 0, value.value['closed']]);
          if (value.value['open'] && value.value['closed'])
            this.issuesActivityChart.dataTable.push([date, value.value['open'], value.value['closed']]);
        });
      } else {
  ...
  getPageViews(span) {
    ...
    let tsTo = this.timeNow;
    this.analyticsService.getMetrics(index, metricType, groupBy, tsFrom, tsTo).subscribe(metrics => {
      this.pageViewsChart.dataTable = [['Date', 'Views']];
      if (!this.isEmpty(metrics.value)) {
