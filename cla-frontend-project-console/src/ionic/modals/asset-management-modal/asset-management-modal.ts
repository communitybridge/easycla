...
export class AssetManagementModal {
  ...
  deleteSelected(event) {
    ...
    let prompt_message = this.selectedFiles
      .map(function(file) {
        return file.name;
      })
      .join(',<br>');
      ...
      if (valid) {
        ...
        reader.onload = (event) => {
          if (uploadMode == 'logo') {
            ...
            this.cincoService
              .obtainDocumentS3URL(this.projectId, this.documentClassifier, docBytes, file.name, contentType)
              .subscribe((response) => {
                if (response.putUrl.url) {
                  let S3URL = response.putUrl.url;
