...
const errorFromResponse = function (res, message) {
  ...
  if (res.body) {
    ...
    if (errorResponse.message) error.userMessage = errorResponse.message;
    if (errorResponse.details) error.userMessage += " " + errorResponse.details;
  }
  return error;
};
  ...
  return {
    ...
    client: function (apiKeys) {
      var makeSignedRequest = function (reqOpts, next) {
        ...
        return sig.signAndMakeRequest(apiKeys, reqOpts, next);
      };
        ...
        addParticipantToMailingList: function (projectId, mailinglistId, newParticipant , next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse(res, 'Unable to add participant to mailinglists with id of [' + mailinglistId + '] from Project [' + projectId + ']'), false);
            }
          });
        },
        ...
        removeParticipantFromMailingList: function (projectId, mailinglistId, participantEmail, next) {
          ...
          makeSignedRequest(opts, function (err, res) {
            if (err) {
              ...
              next(errorFromResponse(res, 'Unable to delete participant [' + participantEmail + '] from mailing list with id of [' + mailinglistId + '] from project with id of [' +
                  projectId + '].'));
            }
          });
        },
        ...
        getParticipantsFromMailingList: function (projectId, mailinglistId, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse(res, 'Unable to get participants from mailing list with id of [' + mailinglistId + '] and project with id of [' + projectId + ']'));
            }
          });
        },
        ...
        getMailingListsAndParticipants: function (projectId, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse(res, 'Unable to get mailing lists & participants from project with id of [' + projectId + ']'));
            }
          });
        }
