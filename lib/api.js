...
const errorFromResponse = function (message, res) {
    ...
    getKeysForLfId: function (lfId, next) {
      request.get(apiRootUrl + "auth/trusted/cas/" + lfId, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse('Unable to get keys for LfId of [' + lfId + '].  ', res));
            } else {
    ...
    getVersion: function (next) {
      request.get(apiRootUrl + 'about/version', function (err, res, body) {
        if (err) {
          ...
          next(errorFromResponse('Unable to get platform version.'), res);
        } else {
        ...
        createUser: function (lfId, next) {
          ...
          makeSignedRequest(opts, function (err, res) {
            if (err) {
              ...
              next(errorFromResponse('User with lfId of [' + lfId + '] not created.',
                  res));
            }
          });
        },
        ...
        getUser: function (id, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse('User with id of [' + id + '] could not be retrieved',
                  res));
            }
          });
        },
        ...
        addGroupForUser: function (id, group, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse('User with id of [' + id + '] could not have group added',
                  res));
            }
          });
        },
        ...
        getAllGroups: function (next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(errorFromResponse('Unable to look up usergroups. ', res));
            }
          });
        },
        ...
        removeGroupFromUser: function (userId, groupId, next) {
          ...
          makeSignedRequest(opts, function (err, res) {
            if (err) {
              ...
              next(errorFromResponse('Unable to delete group with id of [' + groupId + '] from user with id of [' +
                  userId + '].', res));
            }
          });
        },
        ...
        getAllUsers: function (next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(new Error('Unable to get all users.  Status: [' + res.statusCode + '].  ' +
                  'Response Body: ' + body));
            }
          });
        }
