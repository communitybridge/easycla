...
projectStubs['1KJ73l'] = {
  'id': '1KJ73l',
  'name': 'Zephyr',
  'description': 'Ethical beard etsy, locavore ennui pop-up brooklyn yuccie flannel freegan. Roof party brooklyn pug, vice scenester mixtape freegan street art organic chicharrones yuccie. Kitsch hoodie photo booth, mumblecore trust fund bicycle rights occupy raw denim YOLO mixtape small batch salvia swag shoreditch.',
  'url': 'http://www.zephyr.org',
  'pm': 'fvega',
  'type': 'Direct Fund',
  'emailAliases': [],
  'mailingLists': [],
...
projectStubs['2CO89s'] = {
  'id': '2CO89s',
  'name': 'Longer Project Title',
  'description': 'Called they\'re form day, dominion which second every. Had there give herb great. ',
  'url': 'https://www.pyra.com',
  'pm': 'fvega',
  'type': 'Direct Fund',
  'emailAliases': [],
  'mailingLists': [],
...
var projectsArr = [];
projectsArr = projectsArr.concat(projectStubs['1KJ73l'], projectStubs['2CO89s']);
        ...
        getAllUsers: function (next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(res, errorFromResponse('Unable to get all users.  Status: [' + res.statusCode + '].  '));
            }
          });
        },

        getAllProjects: function (next) {
          next(null, projectsArr)
        },

        getProject: function (projectId, next) {
          var proj = projectStubs[projectId];
          if (proj) {
            next(null, proj);
          } else {
            var res = {statusCode: 404, body: ''};
