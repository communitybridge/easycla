    ...
    getKeysForLfId: function (lfId, next) {
      request.get(apiRootUrl + "auth/trusted/cas/" + lfId, function (err, res, body) {
            if (err) {
              ...
              var error = new Error('Unable to get keys for LfId of [' + lfId + '].  ' +
    ...
    getVersion: function (next) {
      request.get(apiRootUrl + 'about/version', function (err, res, body) {
        if (err) {
          ...
          next(new Error('Unable to platform version.  ' +
              'Status: [' + res.statusCode + '].  Response Body: ' + body));
        } else {
        ...
        getUser: function (id, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(new Error('User with id of [' + id + '] could not be retrieved' +
                  'Status: [' + res.statusCode + '].  Response Body: ' + body));
            }
          });
        },
        ...
        addGroupForUser: function (id, group, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(new Error('User with id of [' + id + '] could not have group added' +
                  'Status: [' + res.statusCode + '].  Response Body: ' + body));
            }
          });
        },
        ...
        getAllGroups: function (next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(new Error('Unable to look up usergroups. ' +
                  'Status: [' + res.statusCode + '].  Response Body: ' + body));
            }
          });
        },
        ...
        removeGroupFromUser: function (userId, groupId, next) {
          ...
          makeSignedRequest(opts, function (err, res, body) {
            if (err) {
              ...
              next(new Error('Unable to delete group with id of [' + groupId + '] from user with id of [' +
                  userId + '].  Status: [' + res.statusCode + '].  Response Body: ' + body, false));
            }
          });
        },
