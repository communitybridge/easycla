...
module.exports = {
  ...
  signRequest: signRequestVersionOne,

  // sign using default and then make the requests
  signAndMakeRequest: function signAndMakeRequest(apiKey, requestOpts, callback) {
    request(this.signRequest(apiKey, requestOpts), callback);
  },

  // allow calling either directly (for testing)
  signRequestVersionOne: signRequestVersionOne,
  signRequestVersionTwo: signRequestVersionTwo
...
function signRequestVersionOne(apiKey, requestOpts) {
  ...
  let stringToSign = requestOpts.method + '\n' +
 ...
 * @returns {*} a modified option set.
 */
function signRequestVersionTwo(apiKey, requestOpts) {
