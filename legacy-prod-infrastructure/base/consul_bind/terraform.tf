...
resource "aws_instance" "bind_1" {
  ...
  private_ip             = "${replace(var.cidr, ".0.0/21", ".0.10")}"
...
resource "aws_instance" "bind_2" {
  ...
  monitoring             = true
  user_data              = "${data.template_file.bind-installation-bash.rendered}"
  private_ip             = "${replace(var.cidr, ".0.0/21", ".1.10")}"
...
resource "aws_instance" "bind_3" {
  ...
  monitoring             = true
  user_data              = "${data.template_file.bind-installation-bash.rendered}"
  private_ip             = "${replace(var.cidr, ".0.0/21", ".2.10")}"
...
output "dns_servers" {
  value = ["${replace(var.cidr, ".0.0/21", ".0.10")}", "${replace(var.cidr, ".0.0/21", ".1.10")}", "${replace(var.cidr, ".0.0/21", ".2.10")}"]
}
